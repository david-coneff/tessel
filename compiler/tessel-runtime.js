...
(function(){
  ...
  (function(){
    ...
    function lbl(){
      ...
      var label = tb.querySelector('.btn-label');
      if(label) label.textContent = isLight ? 'Dark' : 'Light';
      else tb.textContent = isLight ? '☾ Dark' : '☀ Light';
    }
  ...
  // ---- IntegrityEngine (AD-T-011) ----
  (function(){
    var badge = document.getElementById('bf-integrity-badge');
    if (!badge) return;
    var CFC_ALGORITHM_LIBRARY = {
    ...
    function parseCfcFilename(name) {
      var m = name.match(/_([0-9a-f]{8})(\.[^.]+)?$/i);
      if (!m) return null;
    ...
    function setBadge(state) {
      ...
      badge.title = {
        verified: 'Integrity verified — file content matches CID in filename',
        mismatch: 'Integrity mismatch — content may have been modified',
        not_canonical: 'Not a CFC filename — no CID to verify against',
        unknown_ver: 'Unknown CFC version in file metadata',
        pending: 'Verifying integrity…'
      }[state] || state;
      badge.textContent = { verified: '✓', mismatch: '⚠', not_canonical: '—', unknown_ver: '?', pending: '…' }[state] || '?';
      badge.className = 'bf-integrity-badge bf-badge-' + state;
    }
    setBadge('pending');
    var cfcVersionMeta = document.querySelector('meta[name="tessel-cfc-version"]');
    var cfcVersion = cfcVersionMeta ? cfcVersionMeta.content : '';
    if (cfcVersion && !CFC_ALGORITHM_LIBRARY[cfcVersion]) { setBadge('unknown_ver'); return; }
    var url = window.location.href;
    var filename = url.split('/').pop().split('?')[0].split('#')[0];
    var cidShort = parseCfcFilename(filename);
    if (!cidShort || !cfcVersion || !CFC_ALGORITHM_LIBRARY[cfcVersion]) { setBadge('not_canonical'); return; }
    fetch(url, {cache: 'no-store'}).then(function(r) {
      return r.arrayBuffer();
    }).then(function(buf) {
      return CFC_ALGORITHM_LIBRARY[cfcVersion].compute(buf);
    }).then(function(hex) {
      setBadge(hex.slice(0, 8) === cidShort ? 'verified' : 'mismatch');
    }).catch(function() { setBadge('not_canonical'); });
  })();
