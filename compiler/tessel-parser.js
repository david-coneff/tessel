 ...
 *   - @field alias (bracket-style alias for @text)
...
function parseFrontMatter(text) {
  var result = { title: '', is_walkthrough: false, collapsible: false, params: [], tags: [], doc_id: '', tessel_cfc_version: '', default_theme: '' };
...
function applyMeta(node, meta) {
  ...
  if (meta.step !== undefined) fieldMeta.step = meta.step;
...
TesselParser.prototype.parse = function(source) {
  ...
  function peekMeta(idx) {
    ...
    if (j < lines.length && lines[j].trim().startsWith('{')) {
      ...
      var depth = 0;
      var start = j;
      while (j < lines.length && (j - start) < 30) {
        var seg = lines[j];
  ...
  return {
    ...
    tessel_cfc_version: frontMatter.tessel_cfc_version || '',
    default_theme: frontMatter.default_theme || '',
