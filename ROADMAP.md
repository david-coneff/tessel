...
**Status:** **In progress.** `studio/tessel-studio-full.html` exists and provides the full-studio foundation including compiler mode. Full WYSIWYG editing and the visual form designer are the remaining work. `dist/tessel-studio-full-standalone.html` is the bundled standalone version.
...
**Goal:** `tools/doc-graph.py` — Rhizome DocGraph protocol (rhiz-docgraph v1) for Merkle-tree document decomposition. Large Markdown artifacts can be split into independently hash-verified section files with a JSON index. Commands: `split`, `update`, `verify`, `merge`, `status`, `init`. Zero external dependencies; Python 3.8+ stdlib only.

**Status:** **Complete.** `tools/doc-graph.py` delivered.

---

### Phase 9 — SVG Toolbar Icons & Compact Mode

**Goal:** Replace emoji toolbar buttons with SVG icon buttons. Add `.btn-label` spans so buttons can run in icon-only compact mode (`body.ts-compact-icons`). Add IntegrityEngine to compiled document runtime (AD-T-011). Add `<meta name="tessel-cfc-version">` to compiler output.
...
Any Tessel component that loads a versioned schema must verify the **entire schema package** before using any of its files.

### AD-T-015 — Versioned executable packages and dependency cascade tooling

Executables and scripts are versioned as **coarse-grained packages** using the same content-addressed folder structure as schemas.
...
| 8 | DocGraph tooling (`tools/doc-graph.py`, rhiz-docgraph v1) | **Complete** | Low |
| 9 | SVG toolbar icons, compact icon-only mode, IntegrityEngine in compiled docs | **In progress** | Low |
...
| `@checkbox[Label\|Opt1\|Opt2]` | `@check[...]` | Phase 1 |
...
**Phase 9 (in progress):** SVG toolbar icons, compact icon-only mode (`body.ts-compact-icons`), IntegrityEngine in compiled document runtime (AD-T-011), `<meta name="tessel-cfc-version">` in compiler output. See `compiler/tessel-compiler.js`, `compiler/tessel-css.js`, `compiler/tessel-runtime.js`.

**Phase 6 (pending Phase 9):** Full WYSIWYG Studio — remaining work is full WYSIWYG editing and the visual form designer.
